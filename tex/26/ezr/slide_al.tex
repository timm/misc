...
5. REVIEW NOTES:
   	- [Tim Note2]: reallt need to make that fr a for else that does a break if nothing found XXX
	- [Tim Note2]: the readons for "cursrot" needs to be explaind.
	- [Tim Note2]: use of local knowledge matters. mistake for all the Y knowledge to be used all the time. mist we use all the x knowledge and just tiny bits of y knowledge (cheating othrwis)
	- [Tim Note2]: need sot show fro ezeeg.py the eval method (seperate train, tst set. train model used to sort
	               test). check the top C things. need my B vs C experiment
	- [Tim Note 2]: cant see acquitewithBayes, acquite with Knn.
	- [Tim Note2:] my add/sub returnt eh added things. so 
	                add(lab, add(best, label(d, unlab.pop(idx)))) works
	= [Tim Note2]: need to adjust code and sub on cols never sub on the main (slow row remvovan;) XXX
	- [Tim Note2: python idiom for for ... break 
