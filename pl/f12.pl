...
xpandShortCut(Head0,Head) :-
  Head0=.. Parts0, append(Tmp,[_,Out],Parts0), append(Tmp,[Out],Parts), Head=.. Parts.

xpandAccessors(Head0 = Fields,T2,[meta(F,At,V0,V,T0,T1)|Access]) :-
  functor(Head0,F,_),
  findall(Access1, xpandAccessor(F=Fields,Access1), [meta(F,At,V0,V,T0,T1)|Access]).
  copy_term(T0,T2).

xpandAccessor(F=L,meta(F,X,V0,V,Term0,Term)) :-
  member(X,L), xpandAccessor(L,V0,V,L0,L,X), Term0=.. [F|L0], Term=.. [F|L].
