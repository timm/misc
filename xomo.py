...
def cocomo2000(i):
  ...
  timE   = tdev()
  staff  = months/timE
  return months,timE,staff,i.risks
...
risks=  dict(
  ...
  sced= dict(cplx=ne46, time=ne46, pcap=nw4, aexp=nw4, acap=nw4,
...
def riskyBusiness(i,risks):
  n=0
  d=i.__dict__
  for a1,t in risks.items():
    for a2,m in t.items(): 
      n += m[d[a1]-1][d[a2]-1] 
  return n 
...
def fill(f, rx=None):
  ...
  i = o(**d)
  tunings = dict(risks=riskyBusiness(i,risks),
