...
# `def add(col, v, inc=1)`
#
# new Num, or updated Sym; inc=-1 undoes
def add(col, v, inc=1):
  if v == "?": return col
...
# `def mids(tbl)`
#
# centroid; only ever read over x columns
def mids(tbl):
...
# `def likes(tbl, row, nall, nh)`
#
# log P(tbl | row), unscaled
def likes(tbl, row, nall, nh):
...
# `def xpect(a, b)`
#
# sizes are >= the.Leaf, so no zero guard
def xpect(a, b):
  return ((div(a)*size(a) + div(b)*size(b))/(size(a) + size(b)))

# `def cutNum(xy, acc)`
#
# (left, right, x) per value boundary
def cutNum(xy, acc):
  xy.sort()
...
# `def _klass(*fits)`
#
# each fit(tbl, rows, y) --> predictor(row)
def _klass(*fits):
...
# `def run(f=None)`
#
# demos may mutate the; always clean up
def run(f=None):
