...
3 & Given generated statistics, plots, or citations, verify each against raw data or the primary source; flag whatever cannot be traced. & \se{510} (prompt reports) --- \emph{partial} & \ding{51} & \ding{51}\\
4 & Given code they did not write, identify defects, judge maintainability, and accept or reject with reasons that survive challenge. & \se{510}, \se{517}, \se{521} (prop.) --- \textbf{ok} & \ding{51} & \\
...
\multicolumn{5}{@{}l}{\emph{What does it cost?}}\\
8 & Given an AI result, benchmark it against at least one non-AI baseline over at least 20 repeated runs, reporting effect size and variance. & \se{510} (M0 eval), 522 --- \emph{partial} & \ding{51} & \ding{51}\\
...
16 & Given a published result, reproduce it from its stated environment, seeds, and scripts. & \se{510} ($\times$2), \se{519} --- \textbf{ok} &  & \\
\multicolumn{5}{@{}l}{\emph{Does it scale? --- up (larger problems), out (more users), down (edge devices)}}\\
...
\vspace{4pt}
\normalsize
\textbf{How the graduate program covers this set.} Stronger than undergrad, as expected: security gains depth (515, 537); evaluation beyond accuracy has a real home (528). \se{510}'s Fall~2026 redesign lifts four rows at course-edit cost --- exactly the mechanism this memo recommends: alien-code review (4) and reproduction (16) are marked twice each across its explore and maintenance projects; a required M0 milestone (pre-registered metric/threshold/baseline eval) houses benchmarking (8); prompt reports score \emph{caught} LLM errors (3, lightly). Scale lands better here too: 21 partial (501, 519), 22 at home (528). Two hard holes remain: experiment design (18 --- research apprenticeship only, so course students never see it) and next-tool-from-docs (20 --- no graduate twin of 491a). Both are what a post-bubble industry pays for; neither needs hardware. The near-approved MS+AI track is the vehicle: write outcomes 8, 18, and 20 into its required core and the graduate program covers the full set without a single new course proposal.
