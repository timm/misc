...
local function o(x,    u)
  if type(x) == "number" then return string.format("%.3g",x) end
  if type(x) ~= "table"  then return tostring(x) end
  u={}
  if #x > 0 then
    for k,v in pairs(x) do u[1+#u] = o(v) end
  else 
    for k,v in pairs(x) do
      if o(k):sub(1,1) ~= "_" then
        u[1+#u] = string.format(":%s %s", k, o(v)) end end end
  return "{" .. table.concat(#x==0 and sort(u) or u," ") .. "}" end
...
function DATA:sort(    fun)
  ...
  self.rows = sorted(self.rows, function(a,b) return fun(a) < fun(b) end)
  return self end
...
function DATA:like(rows, n, nClasses,     col,prior,out,v,inc)
...
function DATA:acquire(rows, score,      todo,done,guess,loop)
  rows  = rows or self.rows
  score = score or function(B,R) return B - R end
  
  function guess(todo,best,rest,n,     t,u,fun,best,rest)
    fun = function(row) return score(best:like(row, n, 2), 
                                     rest:like(row, n, 2)) end
    t,u = {},{}
    for i,row in pairs(todo) do push(t, {i <= the.cut and fun(row) or 0, row}) end
    for _,xy in pairs(sort(t, lt(1))) do push(u,xy[2]) end
    return u end

  todo, done = {}, {}
  for i,row in pairs(rows) do push(i <= the.begin and done or todo, row) end

  while #done < the.Break do
    local best,rest = self:clone(done):bestRest()
		print(#best.rows, #rest.rows)
    todo = guess(todo, best, rest, #done)
    push(done, pop(todo))
