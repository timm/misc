...
function tbl.data(i, t) i.rows[#i.rows + 1] = row.new(t, i.cols) end
...
function tbl.read(i, f)
  for row in lib.csv(f) do tbl.add(i, row) end
  return i end
...
function lib.order(t, i, keys)
  ...
  return function()
    if i < #keys then
      i = i + 1;
      return keys[i], t[keys[i]] end end end

-- Simple print of a flat table
function lib.o(z, pre, s, c)
  s, c = (pre or "") .. '{', ""
  for _, v in lib.order(z or {}) do
    s = s .. c .. tostring(v);
    c = ", " end
  return s .. '}' end
