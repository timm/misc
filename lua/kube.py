...
class Sym(o):
  ...
  def div(i):
    return -sum(v / i.n * math.log(v / i.n, 2)
                for v in i.has.values() if v > 0)
...
class Rows(o):
  ...
  def add(i, row, inc=True, purge=False):
    if purge:
      i._rows.remove(row)  # can be slow. disabled by default
    else:
      i._rows += [row]
    for col in i.cols.all:
      col.add(row[col.at], inc)
    return row
