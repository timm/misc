...
function M.o(x,  n,    t)
  ...
  t={}; for k,v in pairs(x) do 
          if tostring(k):sub(1,1) ~= "_" then 
            t[1+#t] = #x>0 and M.o(v,n) or M.fmt(":%s %s",k,M.o(v,n)) end end 
  return "{" .. M.cat(#x==0 and sort(t) or t," ") .. "}" end
