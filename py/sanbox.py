...
#-------------------------
def okAA(): print(1); return True

def runs(todo,fs=locals().items()):
  use =lambda s: re.match("^ok",s) and (todo==s[2:] or todo==".")
	def run(f)
	  b4 = deepcopy(the)
		random.seed(b4.seed)
	  tmp = f()
		for k,v in b4.items() the[k]=k
  sys.exit(sum(((1 if f()==False else 0) for s,f in fs if use(s))))
