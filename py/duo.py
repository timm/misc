...
def bins4nums(lst, x=0, y=-1, goal=None, cohen=.2,
              enough=.2, trivial=.05):
  """eturn bins for columns of numbers. Combine two bins if
  ...
  def split(xlo=0, bins=[bin0(x, 0)]):
    ...
    for xhi, z in enumerate(lst):
      ...
      now.ys.inc(klass)
      all.ys.inc(klass)
    return [binsScore(bin, all) for bin in bins]
